...
盯价哨兵的 MCP 服务**不是独立运行的**，它依赖于第三方付费 API 数据服务采集的真实竞品数据。请在使用前理解以下**三层数据链路**：

```
安装扩展 → 关注竞品商品链接 → 后端通过付费 API 采集 → AI 对话查询
```
...
① 用户通过扩展关注商品
      │
      ▼
② 后端通过第三方付费 API 获取实时价格 → PostgreSQL
      │
      ▼
...
**关键点：** 数据流向是单向的——用户关注 → 后端通过付费 API 采集 → MCP 读取。扩展只负责告诉系统你要关注哪些商品，不参与实际的价格数据采集。
